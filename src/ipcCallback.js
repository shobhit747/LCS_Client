...
window.electronAPI.receive('connectToServerReply',(directoryInfo,storageInfo,currentPath,hostname)=>{
    changeSection('dashboard.html','mainSection',(complete,err)=>{
        try {
            if(err){throw 'Error : cannot change section'};
            if(complete){
                window.electronAPI.send('getDirInformation',{ directoryInfo : directoryInfo, currentPath : currentPath});
                window.electronAPI.send('getStorageInfo',storageInfo);
                let IpInfo = document.getElementById('ipInfo');
                let div = `
                    <p class="p-1">Network Address : </p>
                    <p class="p-2">${hostname}</p>
                `;
                IpInfo.innerHTML = div;
            }
        } catch (error) {
            popUpMessage({
                message : error,
...
window.electronAPI.receive('fetchDirInformationReply',(data)=>{
    window.electronAPI.send('getDirInformation',data);
})
...
window.electronAPI.receive('getDirInformationReply',(data) =>{
    ...
    for(let fileOrDirectory of Object.keys(directoryInfo)){
        ...
        let tableRow = `
            <button class="tableRow" id="tableRow" path="${fileOrDirectory}" isFile="${isFile}" isFolder="${isFolder}">
                ${svg}
                <p class="p-1">${file}</p>
                <p class="p-2">${size}</p>
            </button>\n
...
window.electronAPI.receive('showFile',(data) =>{
    ...
    if((mimeOne == 'video' || mimeTwo == 'mp4') && mimeOne != 'audio' ){
        let dataTag = `
             <div class="InformationStyle" style="padding:0 0 15px 0;">
                <p class="p-1">Filename</p>
                <p class="p-2">:</p>
                <p class="p-3">
                ${data.fileName}
                </p>
        ...
        let dataTag = `
            ...
            <embed src="${data.url}" frameborder="0" class="fileFrame" type="${data.type}"></embed>
        `
        dirTable.innerHTML = dataTag;
    }
...
window.electronAPI.receive('uploadReply',()=>{
    let currentPath = document.getElementById('currentParhInput');
